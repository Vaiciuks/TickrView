...
function toHeikinAshi(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    result.push({
      time: data[i].time,
      open: haOpen,
      high: Math.max(data[i].high, haOpen, haClose),
      low: Math.min(data[i].low, haOpen, haClose),
      close: haClose,
  ...
  return {
    ...
    crosshair: {
      ...
      vertLine: {
        ...
        width: 1,
        style: 2,
        labelBackgroundColor: tc.crosshairLabel,
      },
      horzLine: {
        color: tc.crosshair,
        width: 1,
...
function hideWatermark(container) {
  requestAnimationFrame(() => {
    ...
    if (el)
      el.style.cssText =
        "position:absolute !important; left:8px !important; right:auto !important; bottom:4px !important; top:auto !important; opacity:0.12 !important; font-size:9px !important; z-index:1 !important;";
  });
}
...
export default function ExpandedChart({
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/insider-trading/${sym}`)
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
      fetch(`/api/options-flow/${sym}`)
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
      fetch(`/api/short-interest/${sym}`)
        .then((r) => (r.ok ? r.json() : null))
        .catch(() => null),
    ])
      ...
      .catch(() => {
        if (!cancelled) setSmartMoneyLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  const isIntraday = ["1m", "2m", "5m", "15m", "30m", "1h"].includes(
    activeTf.interval,
  );
  ...
  const selectMinute = (i) => {
    setMinuteIdx(i);
    setRangeIdx(null);
    fittedTfRef.current = null;
    if (session) saveTimeframe(stock.symbol, i, null);
  };
  const selectRange = (i) => {
    setRangeIdx(i);
    setMinuteIdx(null);
    fittedTfRef.current = null;
    if (session) saveTimeframe(stock.symbol, null, i);
  };
  ...
  useEffect(() => {
    ...
    if (chartType === "line") {
      ...
      mainSeries = chart.addAreaSeries({
        ...
        crosshairMarkerBackgroundColor: "rgba(0,229,255,0.3)",
      });
    ...
    const onTouchStart = (e) => {
      ...
      pinchState.initialDist = Math.abs(
    ...
    const onTouchMove = (e) => {
      ...
      const newTop = Math.max(
        0.01,
        Math.min(0.45, pinchState.initialTop * scale),
      );
      const newBottom = Math.max(
        0.01,
        Math.min(0.45, pinchState.initialBottom * scale),
      );
  ...
  useEffect(() => {
    ...
    if (rsiSeriesRef.current) {
      ...
      if (rsiData.length > 0 && rsiSeriesRef.current._obLine) {
        ...
        rsiSeriesRef.current._osLine.setData(
  ...
  const cancelDraw = useCallback(() => {
    if (trendStartRef.current) {
      ...
      const container = mainContainerRef.current;
      if (container && s.mouseHandler)
        container.removeEventListener("mousemove", s.mouseHandler);
      if (container && s.touchHandler)
        container.removeEventListener("touchmove", s.touchHandler);
  ...
  useEffect(() => {
    ...
    const handler = (param) => {
      ...
      const needsTime = ["trendline", "ray", "fib"].includes(
        drawModeRef.current,
      );
      ...
      if (drawModeRef.current === "hline") {
        ...
        if (!trendStartRef.current) {
          ...
          const svgCircle2 = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "circle",
          );
          ...
          const container = mainContainerRef.current;
          if (container && start.mouseHandler)
            container.removeEventListener("mousemove", start.mouseHandler);
          if (container && start.touchHandler)
            container.removeEventListener("touchmove", start.touchHandler);
          if (start.svg && start.svg.parentNode)
            start.svg.parentNode.removeChild(start.svg);
          if (measureRef.current) measureRef.current.textContent = "";
          ...
          if (mode === "fib") {
            ...
            if (isRay) {
              ...
              try {
                trendSeries.applyOptions({ autoscaleInfoProvider: () => null });
              } catch {}
              setDrawings((prev) => [
                ...prev,
  ...
  const clearDrawings = useCallback(() => {
    ...
    for (const d of drawings) {
      if (d.type === "hline" && d.ref && candleSeriesRef.current) {
        try {
          candleSeriesRef.current.removePriceLine(d.ref);
        } catch {}
      }
      if (
        (d.type === "trend" || d.type === "ray") &&
        d.series &&
        ...
        try {
          mainChartRef.current.removeSeries(d.series);
        } catch {}
      }
    ...
    compareDebounceRef.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(
          `/api/search?q=${encodeURIComponent(compareQuery.trim())}`,
    ...
    const fetchCompare = async () => {
      try {
        const params = new URLSearchParams({
          range: activeTf.range,
          interval: activeTf.interval,
          prepost: String(prepost),
        });
        const res = await fetch(
          `/api/chart/${encodeURIComponent(compareSymbol)}?${params}`,
        );
    ...
    >
      <div className="expanded-header">
        <div className="expanded-header-left">
          ...
          <h2>{stock.symbol}</h2>
          {!compact && stock.name && (
            <span className="expanded-header-name">{stock.name}</span>
          )}
          <div className="expanded-header-actions">
            {onToggleFavorite && (
              <button
                ...
                aria-label={
                  isFavorite
                    ? `Remove ${stock.symbol} from favorites`
                    : `Add ${stock.symbol} to favorites`
                }
              >
                {isFavorite ? "\u2605" : "\u2606"}
              </button>
            ...
            <button
                className="expanded-stats-btn"
                onClick={() => {
                  document.getElementById('section-earnings')?.scrollIntoView({ behavior: 'smooth' });
                }}
                aria-label="Scroll to earnings"
                title="Earnings"
              >
                <svg
                  width="16"
        ...
        <div className="expanded-header-center">
          <span
            className={`expanded-header-price ${isPositive ? "glow-up" : "glow-down"}`}
          >
          ...
          {liveChange != null && (
            ...
            >
              ({isPositive ? "+" : ""}
              {formatPrice(animatedChange)})
            </span>
          )}
          {stock.extPrice != null && (
            <span className="expanded-header-ext">
              <span className="expanded-header-ext-label">
                {stock.extMarketState === "pre" ? "Pre" : "AH"}
              </span>
              {formatPrice(stock.extPrice)}
              <span
                className={
                  stock.extChangePercent >= 0 ? "change-up" : "change-down"
                }
              >
                {" "}
          ...
          <div className="timeframe-groups">
            <div className="timeframe-bar">
              {minuteTfs.map((t, i) => (
                <button
                  key={t.label}
                  className={`timeframe-btn${minuteIdx === i ? " timeframe-active" : ""}`}
            ...
            <div className="timeframe-bar">
              {RANGE_TIMEFRAMES.map((t, i) => (
                <button
                  key={t.label}
                  className={`timeframe-btn${rangeIdx === i && minuteIdx === null ? " timeframe-active" : ""}`}
          ...
          <div className="chart-tools" ref={panelRef}>
              ...
              >
                ...
                >
                  ...
                  <rect
                    ...
                    stroke="none"
                  />
                  <rect x="2.5" y="4" width="4" height="7" rx="0.5" />
              ...
              {activePanel === "indicators" && (
                <div className="chart-tool-panel">
                  ...
                  {[
                    ...
                    >
                      <span className="chart-tool-panel-item-label">
                        {ind.label}
                      </span>
                      <span className="chart-tool-panel-item-desc">
                        {ind.desc}
                      </span>
                    </button>
                  ))}
            ...
            <div className="chart-tool-wrapper">
              <button
                ...
                onClick={() =>
                  setActivePanel((p) => (p === "draw" ? null : "draw"))
                }
                title="Drawing tools"
              >
                <svg
                ...
                <div className="chart-tool-panel">
                  ...
                  {drawings.length > 0 && (
                    <>
                      ...
                      <button
                        className="chart-tool-panel-item danger"
                        onClick={() => {
                          clearDrawings();
                          setActivePanel(null);
                        }}
            ...
            <div className="chart-tool-wrapper">
              <button
                ...
                onClick={() =>
                  setActivePanel((p) => (p === "compare" ? null : "compare"))
                }
              ...
              {activePanel === "compare" && (
                <div className="chart-tool-panel chart-tool-panel--compare">
                  ...
                  {compareSymbol ? (
                    ...
                    <div
                      className="chart-tool-compare-search"
                    ...
                    >
                      <input
                        ...
                        value={compareQuery}
                        onChange={(e) =>
                          setCompareQuery(e.target.value.toUpperCase())
                        }
                        onKeyDown={(e) => {
                          if (e.key === "Enter") {
                            if (
                              compareActiveIdx >= 0 &&
                              compareSuggestions[compareActiveIdx]
                            ) {
                              selectCompare(
                                compareSuggestions[compareActiveIdx].symbol,
                              );
                          ...
                          if (e.key === "ArrowDown") {
                            e.preventDefault();
                            setCompareActiveIdx((i) =>
                              i < compareSuggestions.length - 1 ? i + 1 : 0,
                            );
                          }
                          if (e.key === "ArrowUp") {
                            e.preventDefault();
                            setCompareActiveIdx((i) =>
                              i > 0 ? i - 1 : compareSuggestions.length - 1,
                            );
                          }
                        }}
                        autoFocus
                      ...
                      {compareSuggestions.length > 0 && (
                        <ul className="chart-tool-compare-suggestions">
                          {compareSuggestions.map((item, i) => (
                            ...
                            >
                              <span className="chart-tool-compare-symbol">
                                {item.symbol}
                              </span>
                              <span className="chart-tool-compare-name">
                                {item.name}
                              </span>
                            </li>
                          ))}
            ...
            <div className="draw-mode-badge">
              {drawMode === "hline"
                ...
                : drawClickCount === 0
                  ...
                  : "Click second point"}
              {drawClickCount === 1 && (
                <span ref={measureRef} className="draw-measure" />
              )}
              <button className="draw-mode-cancel" onClick={cancelDraw}>
                &times;
              </button>
            </div>
          )}
      ...
      {!compact && (
        <div className="stock-detail-sections">
          ...
          <div className="stock-detail-section" id="section-earnings">
            ...
            {earningsLoading ? (
              <div className="expanded-stats-loading">Loading earnings data...</div>
            ) : earningsData ? (
              <div className="stock-detail-earnings-content">
                {/* Earnings Profile */}
                <div className="el-profile-header" style={{ padding: "12px 18px 0" }}>
                  <div className="el-profile-badges">
                    {earningsData.streak && earningsData.streak.type !== "none" && earningsData.streak.count > 0 && (
                      <span className={`el-streak-badge ${earningsData.streak.type}`}>
                        {earningsData.streak.count}Q {earningsData.streak.type === "beat" ? "Beat" : earningsData.streak.type === "miss" ? "Miss" : "Met"} Streak
                      </span>
                    )}
                    {earningsData.nextEarningsDate && (
                      <span className="el-next-earnings">Next: {earningsData.nextEarningsDate}</span>
                    )}
                  </div>
                </div>
                      ...
                      {earningsData.highlights.map((h, i) => (
                        <div key={i} className="el-highlight-item">
                          ...
                          <div className="el-highlight-content">
                            <span className="el-highlight-title">{h.title}:</span> {h.detail}
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              <div className="expanded-stats-loading">No earnings data available</div>
            )}
          </div>
