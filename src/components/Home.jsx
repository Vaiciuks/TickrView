...
function getMarketStatus() {
  ...
  if (holiday) return { status: 'closed', label: 'Markets Closed', sub: holiday.name };
  if (isWeekend) return { status: 'closed', label: 'Markets Closed', sub: 'Weekend' };
  if (mins >= 570 && mins < 960) return { status: 'open', label: 'Markets Open', sub: 'Regular Session' };
  if (mins >= 240 && mins < 570) return { status: 'pre', label: 'Pre-Market', sub: 'Opens at 9:30 AM ET' };
  if (mins >= 960 && mins < 1200) return { status: 'after', label: 'After-Hours', sub: 'Regular session ended' };
    ...
    <div className="skeleton-stock-list">
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className="skeleton-row">
          ...
          <div className="skeleton-line" style={{ width: 44, height: 18, borderRadius: 9 }} />
        </div>
      ))}
    </div>
  );
    ...
    <div className="skeleton-movers-split">
      {[0, 1].map(col => (
        <div key={col} className="skeleton-movers-col">
          <div className="skeleton-line" style={{ width: 50, height: 10, marginBottom: 4 }} />
          {Array.from({ length: 3 }, (_, i) => (
            <div key={i} className="skeleton-row">
              <div className="skeleton-circle" style={{ width: 16, height: 16 }} />
              <div className="skeleton-line" style={{ width: 36, height: 12 }} />
              <div className="skeleton-line" style={{ width: 38, height: 16, borderRadius: 8, marginLeft: 'auto' }} />
            </div>
          ))}
        </div>
      ))}
...
function SkeletonEarnings() {
  return (
    <>
      <div className="skeleton-line" style={{ width: 160, height: 10, marginBottom: 8 }} />
...
function SkeletonNews() {
  return (
    <div className="skeleton-news-layout">
      <div className="skeleton-line skeleton-news-featured" style={{ minHeight: 240 }} />
      <div className="skeleton-news-side">
        {Array.from({ length: 4 }, (_, i) => (
          <div key={i} className="skeleton-news-item">
            <div className="skeleton-line" style={{ width: 60, height: 44, borderRadius: 6, flexShrink: 0 }} />
  ...
  return (
    <>
      <div className="skeleton-hero-main">
        ...
        <div className="skeleton-line skeleton-hero-chart" style={{ width: 200, height: 48 }} />
      </div>
      <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: 14, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        {Array.from({ length: 4 }, (_, i) => (
...
function SkeletonGauge() {
  return (
    <div className="skeleton-gauge">
      <div className="skeleton-circle" style={{ width: 160, height: 80, borderRadius: '80px 80px 0 0' }} />
      <div className="skeleton-line" style={{ width: 80, height: 14 }} />
...
function SectionCard({ title, tabKey, onNavigate, children, className = '' }) {
  return (
    <div className={`home-card ${className}`} onClick={() => onNavigate(tabKey)} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') onNavigate(tabKey); }}>
...
function StockRow({ stock, rank }) {
  ...
  return (
    <div className={`home-stock-row ${flash || ''}`}>
      ...
      <span className={`home-stock-price ${flash || ''}`}>{formatPrice(animatedPrice)}</span>
      <span className={`home-badge ${isPos ? 'home-badge--green' : 'home-badge--red'}`}>
...
function Sparkline({ data, width = 120, height = 32, isPositive = true, hero = false }) {
  if (!data || data.length < 2) return <div style={{ width: '100%', height }} />;
      ...
      {hero && (
        <defs>
          <linearGradient id={gradId} x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor={isPositive ? 'rgba(0,200,83,0.2)' : 'rgba(255,23,68,0.2)'} />
...
function HeroChange({ stock }) {
  ...
  return (
    <span className={`hero-change ${isPos ? 'change-up' : 'change-down'}`}>
      {isPos ? '+' : ''}{animatedChange.toFixed(2)}
      {' '}({isPos ? '+' : ''}{animatedPercent.toFixed(2)}%)
    </span>
  );
}
...
function SecondaryIndex({ stock, label, sparkData }) {
  ...
  return (
    <div className={`hero-secondary ${flash || ''}`}>
      ...
      <div className={`hero-secondary-price ${flash || ''}`}>{formatPrice(animatedPrice)}</div>
...
function MarketPulseCard({ futures, sparklines, onNavigate }) {
  const INDEX_MAP = { 'ES=F': 'S&P 500', 'NQ=F': 'Nasdaq', 'YM=F': 'Dow Jones', 'RTY=F': 'Russell 2000', '^VIX': 'VIX' };
  const SECONDARY_ORDER = ['NQ=F', 'YM=F', 'RTY=F', '^VIX'];

  const sp = futures.find(s => s.symbol === 'ES=F');
  const isUp = sp ? sp.changePercent >= 0 : true;
  const secondaryIndices = SECONDARY_ORDER.map(sym => futures.find(s => s.symbol === sym)).filter(Boolean);
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      onKeyDown={e => { if (e.key === 'Enter') onNavigate('futures'); }}
    >
      <div className="home-card-header">
        <span className="home-card-title">
          <span className={`hero-status-dot hero-status-dot--${mktStatus.status}`} />
          <span className="hero-status-label" title={mktStatus.sub}>
            {mktStatus.sub && mktStatus.sub !== 'Weekend' && !mktStatus.sub.startsWith('Opens') && !mktStatus.sub.startsWith('Regular')
...
function NewsSection({ articles, onNavigate }) {
  ...
  if (articles.length === 0) {
    return (
      <div className="home-news-section" onClick={() => onNavigate('news')} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') onNavigate('news'); }}>
  ...
  return (
    <div className="home-news-section">
      <div className="home-section-header" onClick={() => onNavigate('news')} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') onNavigate('news'); }}>
        <span className="home-section-title">Latest News</span>
        <span className="home-card-link">View All &rarr;</span>
      </div>
      <div className="home-news-grid">
        {featured && (
          ...
          >
            <div className="home-news-featured-img" style={{ backgroundImage: `url(${featured.thumbnail})` }} />
            ...
            <div className="home-news-featured-content">
              ...
              {featured.publishedAt && (
                <span className="home-news-featured-time">{timeAgo(featured.publishedAt)}</span>
              )}
            </div>
          </a>
        )}
        <div className="home-news-side">
          {sideArticles.map((a, i) => (
            ...
            >
              {a.thumbnail && (
                <div className="home-news-side-thumb" style={{ backgroundImage: `url(${a.thumbnail})` }} />
              )}
              <div className="home-news-side-text">
                ...
                <span className="home-news-side-meta">
                  ...
                  {a.publishedAt && <span className="home-news-time">{timeAgo(a.publishedAt)}</span>}
                </span>
              </div>
            </a>
          ))}
...
function EarningsPreview({ earnings, onNavigate }) {
  const upcoming = useMemo(() => {
    ...
    allStocks.sort((a, b) => a.date.localeCompare(b.date) || (b.marketCap || 0) - (a.marketCap || 0));
    return allStocks;
  }, [earnings]);
  ...
  return (
    <SectionCard title="Earnings" tabKey="earnings" onNavigate={onNavigate}>
      {Object.keys(earnings).length === 0 ? (
        ...
        <>
          <div className="home-card-stat">{weekCount} companies reporting soon</div>
          <div className="home-stock-list">
            {top3.map(s => (
              <div key={s.symbol} className="home-stock-row">
                ...
                <span className="home-earnings-date">{new Date(s.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
              </div>
            ))}
          </div>
        </>
          ...
          {upcoming.map(e => (
            <div key={e.id} className="home-econ-item">
              ...
              <span className="home-econ-date">
                {new Date(e.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
              </span>
            </div>
          ))}
        </div>
...
function FearGreedGauge({ active, futures }) {
  ...
  const vixStock = futures.find(s => s.symbol === '^VIX' || s.symbol === 'VX=F');
  const vixPrice = vixStock?.price || null;

  // VIX component: VIX 10 → 100 (extreme greed), VIX 35 → 0 (extreme fear), linear
  const vixScore = vixPrice != null ? Math.max(0, Math.min(100, ((35 - vixPrice) / 25) * 100)) : null;

  // Breadth component: 80%+ advancing → 100, 20%- → 0
  const breadthScore = breadthData
    ? Math.max(0, Math.min(100, ((breadthData.advancing / breadthData.total - 0.2) / 0.6) * 100))
    : null;
  ...
  return (
    <div className="home-feargreed">
      ...
      <div className="home-feargreed-factors">
        ...
        {breadthData && (
          <span className="home-feargreed-factor">
            Breadth: {((breadthData.advancing / breadthData.total) * 100).toFixed(0)}%
          </span>
        )}
      </div>
    </div>
  ...
  useEffect(() => {
    ...
    fetch('/api/heatmap')
      ...
      .then(json => {
        ...
        const total = allStocks.length;
        const adRatio = declining > 0 ? (advancing / declining).toFixed(2) : 'N/A';
  ...
  return (
    <div className="home-breadth-section">
      ...
      <div className="home-breadth-bar">
        ...
        <div className="home-breadth-bar-unch" style={{ width: `${unchPct}%` }} />
        <div className="home-breadth-bar-dec" style={{ width: `${decPct}%` }} />
      </div>
      <div className="home-breadth-stats">
        <span className="home-breadth-stat home-breadth-stat--adv">{advancing} Advancing</span>
        <span className="home-breadth-stat home-breadth-stat--unch">{unchanged} Unchanged</span>
        <span className="home-breadth-stat home-breadth-stat--dec">{declining} Declining</span>
...
function HeatmapPreview({ active, onNavigate }) {
  ...
  const sectorData = useMemo(() => {
    ...
    return sectors.map(s => {
      ...
      const totalCap = stocks.reduce((sum, st) => sum + (st.marketCap || 0), 0);
      const weightedChange = totalCap > 0
        ? stocks.reduce((sum, st) => sum + (st.changePercent || 0) * (st.marketCap || 0), 0) / totalCap
        : 0;
      return { name: s.name, change: weightedChange, cap: totalCap, count: stocks.length };
    }).filter(s => s.cap > 0).sort((a, b) => b.cap - a.cap);
  }, [sectors]);

  return (
    <div className="home-heatmap-section">
      <div className="home-section-header" onClick={() => onNavigate('heatmap')} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') onNavigate('heatmap'); }}>
        <span className="home-section-title">Market Heatmap</span>
        <span className="home-card-link">Full View &rarr;</span>
      </div>
      {sectorData.length > 0 ? (
        <div className="home-heatmap-grid" onClick={() => onNavigate('heatmap')} role="button" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') onNavigate('heatmap'); }}>
          {sectorData.map(s => (
            <div
              key={s.name}
            ...
            >
              ...
              <span className={`home-heatmap-cell-change ${s.change >= 0 ? 'change-up' : 'change-down'}`}>
...
function MoverRow({ stock, isGainer }) {
  ...
  return (
    <div className={`home-movers-row ${flash || ''}`}>
      ...
      <span className="home-stock-symbol">{stock.symbol}</span>
      <span className={`home-badge ${isGainer ? 'home-badge--green' : 'home-badge--red'}`}>
        {isGainer ? '+' : ''}{animatedChange.toFixed(1)}%
      </span>
    </div>
  );
}
...
function FuturesRow({ stock, label }) {
  ...
  return (
    <div className={`home-futures-row ${flash || ''}`}>
      ...
      <span className={`home-stock-price ${flash || ''}`}>{formatPrice(animatedPrice)}</span>
      <span className={`home-badge home-badge--sm ${isPos ? 'home-badge--green' : 'home-badge--red'}`}>
        {isPos ? '+' : ''}{animatedPercent.toFixed(2)}%
      </span>
    </div>
  );
      ...
      {hasData ? (
        <div className="home-movers-split">
          <div className="home-movers-col">
            ...
            {topG.map(s => <MoverRow key={s.symbol} stock={s} isGainer />)}
          </div>
          <div className="home-movers-col">
            <span className="home-movers-label home-movers-label--down">Losers</span>
            {topL.map(s => <MoverRow key={s.symbol} stock={s} isGainer={false} />)}
          </div>
        </div>
      ) : (
...
export default function Home({ active, gainers = [], losers = [], trending = [], futures = [], crypto = [], favorites = [], onTabChange }) {
  ...
  const FUTURE_NAMES = { 'ES=F': 'S&P 500', 'NQ=F': 'Nasdaq', 'YM=F': 'Dow', 'CL=F': 'Crude Oil', 'GC=F': 'Gold' };
  ...
  const startDrag = useCallback((key, x, y) => {
    if (!gridRef.current) return;
    dragRef.current.cancelled = false;
    dragRef.current.active = true;

    // Source element + label for ghost
    const sourceEl = gridRef.current.querySelector(`[data-section-key="${key}"]`);
    if (!sourceEl) return;
    ...
    const offX = ghostW / 2, offY = ghostH / 2;
    ...
    const frame = () => {
      ...
      if (dirty) {
        ...
        if (foundEl !== currentOverEl) {
          if (currentOverEl) currentOverEl.classList.remove('home-section--dragover');
          currentOverEl = foundEl;
          if (currentOverEl) currentOverEl.classList.add('home-section--dragover');
        }
        dirty = false;
      }
  ...
  const handlePointerDown = useCallback((e) => {
    if (e.target.closest('.home-section-controls-btns') || e.target.closest('.home-section-ctrl-btn')) return;
    if (e.target.closest('button') || e.target.closest('a')) return;
    const wrapper = e.currentTarget.closest('[data-section-key]') || e.currentTarget;
  ...
  const renderSection = (key) => {
    switch (key) {
      case 'pulse':
        return <MarketPulseCard futures={futures} sparklines={sparklines} onNavigate={onTabChange} />;
      case 'runners':
        return (
          <SectionCard title="Top Runners" tabKey="gainers" onNavigate={onTabChange}>
            {gainers.length > 0 ? (
              <div className="home-stock-list">
                {gainers.slice(0, 4).map((s, i) => <StockRow key={s.symbol} stock={s} rank={i + 1} />)}
              </div>
            ) : <SkeletonStockList />}
          </SectionCard>
        );
      ...
      case 'losers':
        return (
          <SectionCard title="Top Losers" tabKey="losers" onNavigate={onTabChange}>
            {losers.length > 0 ? (
              <div className="home-stock-list">
                {losers.slice(0, 4).map((s, i) => <StockRow key={s.symbol} stock={s} rank={i + 1} />)}
              </div>
            ) : <SkeletonStockList />}
          </SectionCard>
        );
      ...
      case 'trending':
        return (
          <SectionCard title="Trending" tabKey="trending" onNavigate={onTabChange}>
            {trending.length > 0 ? (
              <div className="home-stock-list">
                {trending.slice(0, 4).map(s => <StockRow key={s.symbol} stock={s} />)}
              </div>
            ) : <SkeletonStockList />}
          </SectionCard>
        );
      case 'futures':
        return (
          <SectionCard title="Futures" tabKey="futures" onNavigate={onTabChange}>
            {keyFutures.length > 0 ? (
              <div className="home-futures-list">
                {keyFutures.map(s => (
                  <FuturesRow key={s.symbol} stock={s} label={FUTURE_NAMES[s.symbol] || s.symbol} />
                ))}
              </div>
            ) : <SkeletonFuturesList />}
          </SectionCard>
      ...
      case 'crypto':
        return (
          <SectionCard title="Crypto" tabKey="crypto" onNavigate={onTabChange}>
            {crypto.length > 0 ? (
              <div className="home-stock-list">
                {crypto.slice(0, 4).map(s => <StockRow key={s.symbol} stock={s} />)}
              </div>
            ) : <SkeletonStockList />}
          </SectionCard>
        );
  ...
  return (
    <main className="home-main">
      ...
      <div className="home-brand">
        ...
        <p className="home-brand-tagline">Real-time market intelligence at your fingertips</p>
      </div>
      ...
      {editMode && (
        <div className="home-edit-toolbar">
          <div className="home-edit-toolbar-inner">
            <button className="home-reset-btn" onClick={resetLayout}>Reset to Default</button>
            <span className="home-edit-toolbar-label">Editing Layout</span>
            <button className="home-edit-done-btn" onClick={() => setEditMode(false)}>Done</button>
          </div>
          {hiddenSections.length > 0 && (
            <div className="home-hidden-inline">
              ...
              {hiddenSections.map(key => {
                ...
                return (
                  <button key={key} className="home-hidden-item" onClick={() => toggleVisibility(key)}>
                    {section.label}
      ...
      <div className="home-sections-grid" ref={gridRef}>
        {visibleSections.map(key => {
          ...
          return (
            ...
            >
              {editMode && (
                <div className="home-section-controls" onClick={e => e.stopPropagation()}>
                  <span className="home-section-controls-label">{section.label}</span>
                  <div className="home-section-controls-btns">
                    <button onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()} onClick={() => toggleVisibility(key)} title="Hide section" className="home-section-ctrl-btn home-section-ctrl-btn--hide">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19M1 1l22 22"/></svg>
                    </button>
                  </div>
                </div>
              )}
